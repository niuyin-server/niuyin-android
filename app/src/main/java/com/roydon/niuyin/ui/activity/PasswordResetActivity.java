...
import com.roydon.niuyin.http.request.PasswordApi;
...
public final class PasswordResetActivity extends MyActivity {
    ...
    protected void initView() {
        InputTextHelper
                ...
                .setMain(mCommitView)
                .setListener(helper -> mPasswordView1.getText().toString().length() >= 6 && mPasswordView1.getText().toString().equals(mPasswordView2.getText().toString()))
                .build();
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.btn_password_reset_commit) {
            ...
            // 重置密码
            EasyHttp.post(this).api(new PasswordApi().setPhone(mPhone).setCode(mCode).setPassword(mPasswordView1.getText().toString())).request(new HttpCallback<HttpData<Void>>(this) {
